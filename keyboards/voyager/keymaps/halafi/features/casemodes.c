...
static bool caps_word_on_local = false;
...
bool caps_word_enabled(void) {
    return caps_word_on_local;
}

// Enable caps word
void enable_caps_word(void) {
    caps_word_on_local = true;
...
bool process_case_modes(uint16_t keycode, const keyrecord_t *record) {
    if (caps_word_on_local || xcase_state) {
        ...
        if (record->event.pressed) {
            ...
            else if (caps_word_on_local && keycode >= KC_A && keycode <= KC_Z){
